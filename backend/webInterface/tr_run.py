...
class TrRun(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        logger.info(f"pre-processing time: {pre_end - pre_start}s")

        # 进行ocr
        inf_start = time.monotonic()
        res = tr.run(img.copy().convert("L"), flag=tr.FLAG_ROTATED_RECT)
        inf_end = time.monotonic()
        logger.info(f"inference time: {inf_end - inf_start}s")
        ...
        logger.info(json.dumps(new_log_info, cls=NpEncoder))
